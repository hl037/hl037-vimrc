...
function M.show_memory(addr_expr, size_expr, config)
  ...
  M.evaluate_number({expr=addr_expr, evaluator=evaluator, cb=function(address, err)
    if err ~= nil then
      vim.notify(err, vim.log.levels.ERROR)
      return
    end
    M.evaluate_number({expr=size_expr, evaluator=evaluator, cb=function(size, err)
      if err ~= nil then
        vim.notify(err, vim.log.levels.ERROR)
        return
      end
      local buffer = require'dap-mem-view.views'.MemView:new(addr_expr, address, size, config)
      buffer:show()
    end})
  end})
end
...
function M.evaluate_number(args)
  ...
  if type(expr) == "number" then
    ...
    if args.frameId then
      req.frameId = args.frameId
    end

    dap.session():request(cmd, req, function(err, resp)
      if err then
        cb(nil, "Evaluate error: " .. err.message)
        return
      end
      local val = evaluator.parse_response(resp)
      if not val then
        cb(nil, "Could not parse evaluate result: " .. vim.inspect(resp))
        return
      end
      cb(val)
    end)
